...
class Prompting(object):
  ...
  def compute_tokens_prob(self, text, token_list1, token_list2, token_list3, token_list4, token_list5):
    """
    ...
    baseline = [2.6861, 8.1680, 5.5605,8.1387,5.0097]
    ...
    score3=[self.scores_dict[token3] if token3 in self.scores_dict.keys() else 0\
            for token3 in token_list3]
    score3= sum(score3)

    score4=[self.scores_dict[token4] if token4 in self.scores_dict.keys() else 0\
            for token4 in token_list4]
    score4= sum(score4)

    score5=[self.scores_dict[token5] if token5 in self.scores_dict.keys() else 0\
            for token5 in token_list5]
    score5= sum(score5)

    print([score1,score2,score3,score4,score5])
    softmax_rt=torch.nn.functional.softmax(torch.Tensor([score1,score2,score3,score4,score5])-torch.Tensor(baseline), dim=0)
    # print(softmax_rt)
    return softmax_rt
